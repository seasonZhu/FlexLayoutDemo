...
class NovelSummaryVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        coverImgV.rx.tapGesture().subscribe(onNext:  { [weak self] _ in
            guard let self else {
                return
            }
            
            if self.descContainer.isHidden {
                self.descContainerShow()
            } else {
                self.descContainerHidden()
            }
        ...
        rootFlex.flex.marginTop(88).define { flex in
            ...
            /// 把这个盒子移到了外面,这样显示效果才正常 浅红色盒子, padding(上, 左, 下, 右)
    ...
    private func descContainerHidden() {
        descContainer.flex.isIncludedInLayout = false
        descContainer.isHidden = true
        view.setNeedsLayout()
    }
    
    private func descContainerShow() {
        descContainer.flex.isIncludedInLayout = true
